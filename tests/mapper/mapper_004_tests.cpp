...
TEST(Mapper004Test, UpdatesPrgBanksAndIrqState) {
    ...
    EXPECT_TRUE(mapper.cpuMapWrite(0x8000, mapped_addr, 0x06));
    EXPECT_TRUE(mapper.cpuMapWrite(0x8001, mapped_addr, 0x03));
    EXPECT_TRUE(mapper.cpuMapRead(0x8000, mapped_addr, data));
    EXPECT_EQ(mapped_addr, 0x6000u);

    EXPECT_TRUE(mapper.cpuMapWrite(0xA000, mapped_addr, 0x01));
    EXPECT_EQ(mapper.mirrorMode(), 3u);

    EXPECT_TRUE(mapper.cpuMapWrite(0xC000, mapped_addr, 0x01));
    EXPECT_TRUE(mapper.cpuMapWrite(0xE001, mapped_addr, 0x00));
    mapper.scanline();
